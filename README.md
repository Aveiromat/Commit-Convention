...
Este repositório é uma **ferramenta auxiliar** criada para ser utilizada em vários tipos de projetos. Ele define nossa convenção de commits e contém o script `commit-msg.sh`, um Git Hook personalizado criado para garantir que qualquer projeto siga estritamente o padrão de **Conventional Commits com Emojis**.
...
Como este é um repositório auxiliar, o objetivo é levar o arquivo `commit-msg.sh` para os seus novos projetos e configurá-lo como um Git Hook para validar as mensagens de commit localmente.

### Passo 1: Certifique-se de que o Git está instalado 🌟

Antes de tudo, verifique se o Git está instalado na máquina do projeto destino. Abra o terminal e execute:

```bash
git --version
```

### Passo 2: Obtenha o arquivo `commit-msg.sh` 📂

Copie o arquivo `commit-msg.sh` deste repositório auxiliar para o novo projeto. Você pode clonar este repositório ou baixá-lo diretamente.

Por exemplo:

```bash
git clone https://github.com/Aveiromat/Commit-Convention.git
cp Commit-Convention/commit-msg.sh /caminho/para/seu/novo-projeto/
```

### Passo 3: Vá para o seu novo projeto e crie o diretório `.git/hooks` 📁

No terminal, navegue até a raiz do seu novo projeto. Os hooks do Git ficam no diretório `.git/hooks`. Verifique se ele existe e crie-o se necessário:
...
Copie o arquivo `commit-msg.sh` que você obteve no passo anterior para o diretório `.git/hooks` do seu projeto e renomeie-o para `commit-msg` (sem a extensão `.sh`):
...
Para que o Git possa executar o script sempre que você tentar fazer um commit, você precisa dar permissão de execução:
...
Se a mensagem de commit seguir o padrão definido (com o devido emoji e estrutura), o commit será aceito. Caso contrário, o hook irá bloquear o commit e exibir uma mensagem de erro indicando como consertar.

> **Nota:** Commits de Merge (ex: `Merge branch...`) são automaticamente ignorados pelo nosso script para não quebrar fluxos de trabalho do Git.

### Passo 7: Documentação no novo projeto (opcional) 🎨

Recomendamos copiar as regras de uso destas convenções para o README do seu novo projeto (ou criar um arquivo `COMMIT_CONVENTION.md`) para que o restante da equipe saiba o padrão correto.
